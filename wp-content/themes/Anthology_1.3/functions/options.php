...
"type" => "text",
"std" => get_bloginfo('template_directory').'/images/serv_icon2.png',
"upload" => true
),
...
"type" => "text",
"std" => get_bloginfo('template_directory').'/images/serv_icon3.png',
"upload" => true
),
...
"type" => "text",
"std" => get_bloginfo('template_directory').'/images/serv_icon.png',
"upload" => true
),
...
"type" => "select",
"options" => array('Random', 'Fold', 'Fade', 'Slice Down'),
"values" => array('random', 'fold', 'fade', 'sliceDown'),
"std" => 'random'
),
...
<img class="option-image" src="<?php bloginfo('template_url');?>/images/<?php echo $value['image'];?>" alt="image" />
</div>
	<?php } ?> 
...
<?php } ?>
	...
	<?php if($value['color']==true){?>
	 <div class="color-preview" style="background-color:#<?php echo get_settings( $value['id'] ); ?>"></div>
	 <?php } ?>
<br />
...
case 'hidden':
	...
	value="<?php if ( get_settings( $value['id'] ) != "" && get_settings( $value['id'] ) != null) { echo stripslashes(get_settings( $value['id'] )); } else { echo stripslashes($value['std']); } ?>" />
...
<img class="option-image" src="<?php bloginfo('template_url');?>/images/<?php echo $value['image'];?>" alt="image" />
	...
	<?php } ?> <textarea name="<?php echo $value['id']; ?>" class="option-textarea" cols="" rows=""><?php if ( get_settings( $value['id'] ) != "") { echo stripslashes(get_settings( $value['id'] )); } else { echo stripslashes($value['std']); } ?>
</textarea> <br />
<span class="option-description"><?php echo $value['desc']; ?></span></div>
	<?php
	break;
...
<img class="option-image" src="<?php bloginfo('template_url');?>/images/<?php echo $value['image'];?>" alt="image" />
	...
	if($value['pattern']) {
		$pexeto_pattern_path=get_bloginfo('template_url').'/images/patterns';
