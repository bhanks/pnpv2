...
class GFCommon{
    ...
    private static function get_post_image_variable($media_id, $arg1, $arg2){
    ...
    public static function replace_variables_post_image($text, $post_images, $lead){
        ...
        if(is_array($matches))
        {
            foreach($matches as $match){
                ...
                $value = is_wp_error($media_id) ? "" : self::get_post_image_variable($media_id, $size_meta, $align);
            ...
            foreach($matches as $match){
                ...
                switch(RGFormsModel::get_input_type($field)){
                    ...
                    case "radio" :
                        ...
                        if(!$use_value)
                            $value = RGFormsModel::get_choice_text($field, $raw_value);
                    break;
    ...
    public static function get_field_input($field, $value="", $lead_id=0, $form_id=0){
        ...
        switch(RGFormsModel::get_input_type($field)){
            ...
            case "post_custom_field":
                $tabindex = self::get_tabindex();
                return !empty($post_link) ? $post_link : sprintf("<div><input name='input_%d' id='%s' type='text' value='%s' class='%s' $tabindex %s/></div>", $id, $field_id, esc_attr($value), esc_attr($class), $disabled_text);
            break;

            case "post_content":
            case "post_excerpt":
                $tabindex = self::get_tabindex();
                return !empty($post_link) ? $post_link : sprintf("<div><textarea name='input_%d' id='%s' class='textarea %s' $tabindex %s rows='10' cols='50'>%s</textarea></div>", $id, $field_id, esc_attr($class), $disabled_text, esc_html($value));
            break;
...
*/
            ...
            case "time" :
                ...
                return sprintf("<div class='clear-multi'><div class='gfield_time_hour ginput_container' id='%s'><input type='text' maxlength='2' name='input_%d[]' id='%s.1' value='%s' $hour_tabindex %s/> : <label for='%s.1'>" . __("HH", "gravityforms") . "</label></div><div class='gfield_time_minute'><input type='text' maxlength='2' name='input_%d[]' id='%s.2' value='%s' $minute_tabindex %s/><label for='%s.2'>" . __("MM", "gravityforms") . "</label></div><div class='gfield_time_ampm'><select name='input_%d[]' id='%s.3' $ampm_tabindex %s><option value='am' %s>" . __("AM", "gravityforms") . "</option><option value='pm' %s>" . __("PM", "gravityforms") . "</option></select></div></div>", $field_id, $id, $field_id, $hour, $disabled_text, $field_id, $id, $field_id, $minute, $disabled_text, $field_id, $id, $field_id, $disabled_text, $am_selected, $pm_selected);
